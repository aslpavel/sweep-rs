...
pub struct SweepInner<H: Haystack> {
    ...
    response: channel::Receiver<SweepResponse<H>>,
            ...
            "haystack_extend" => {
                ...
                for item in items {
                    if let Value::String(string) = item {
                        candidates.push(Candidate::new(string, delimiter, field_selector));
                    } else {
                        let error = request.response_err(
                            RPCErrorKind::InvalidParams,
                            Some("[haystack_extend] item must be a string"),
                        );
                        return Some(error);
                    }
                }
...
fn sweep_worker<H>(
    ...
    response: channel::Sender<SweepResponse<H>>,

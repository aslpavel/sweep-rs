use crate::{FuzzyScorer, Haystack, Ranker, RankerResult, ScoreResult, ScorerBuilder};
use anyhow::Error;
    ...
    H: Haystack,
{
    ...
    fn send_command(&self, command: SweepCommand) -> Result<(), Error> {
    ...
    pub fn niddle_set(&self, niddle: impl AsRef<str>) -> Result<(), Error> {
    ...
    pub fn prompt_set(&self, prompt: String) -> Result<(), Error> {
    ...
    pub fn bind(&self, chord: Vec<Key>, tag: Value) -> Result<(), Error> {
        self.send_command(SweepCommand::Bind(chord, tag))?;
        Ok(())
    }

    /// Event generated by the `Sweep` object
    pub fn events(&self) -> &channel::Receiver<SweepEvent<H>> {
        &self.events
    }
}
...
pub struct SweepInner<H: Haystack> {
    ...
    commands: channel::Sender<SweepCommand>,
    events: channel::Receiver<SweepEvent<H>>,
}
...
impl<H: Haystack> SweepInner<H> {
    pub fn new(options: SweepOptions) -> Result<Self, Error> {
        ...
        let worker = Builder::new().name("sweep_worker".to_string()).spawn({
            ...
            move || sweep_worker(options, term, ranker, commands_recv, events_send)
        })?;
        Ok(SweepInner {
            ranker,
            waker,
...
fn sweep_worker<H>(
    ...
    ranker: Ranker<H>,
    commands: channel::Receiver<SweepCommand>,
    events: channel::Sender<SweepEvent<H>>,
) -> Result<(), Error>
where
    H: Haystack,
{
                    ...
                    for command in commands.try_iter() {
                        match command {
                            ...
                            SweepCommand::Current => {
                                ...
                                events.send(SweepEvent::Current(current))?;
                            }
                        }
                    }
                }
